...
test('summarize: frame + phase stats and metadata', () => {
    ...
    assert.equal(s.frame.avg, 16);
    assert.equal(s.frame.fps, 1000 / 16);
    assert.equal(s.frame.jankRatio, 0.25);
    assert.equal(s.frame.spikeRatio, 0.25);
    assert.equal(s.frame.frameClass, 'throttled');
    ...
    assert.equal(s.phases.render.avg, 3.5);
});
...
test('checkRegression: catches worsening, passes identical', () => {
    ...
    const bad = checkRegression(base, cand);
...
test('assertNoRegression: throws on regression, returns on clean', () => {
    ...
    try { assertNoRegression(base, cand); } catch (e) {
...
test('litecap: v1 buffers still decode (tags empty, meta null)', () => {
    ...
    v.setUint8(0, 0x4C); v.setUint8(1, 0x43); v.setUint8(2, 0x41); v.setUint8(3, 0x50);
    v.setUint8(4, 1);
    v.setUint32(5, count, true);
    v.setUint8(9, 0);
    const vals = [8, 16, 40];
    let off = 10;
